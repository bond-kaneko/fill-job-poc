...
// InsertOutbox は同一 task_id の二重 enqueue を UNIQUE 制約で 1 行に丸める。
func InsertOutbox(ctx context.Context, q Querier, taskID string, body []byte) error {
	_, err := q.Exec(ctx, `
		INSERT INTO outbox (task_id, body)
		VALUES ($1, $2)
...
// ClaimOutbox は relay 用に未送信の 1 件を select for update skip locked で取る。
func ClaimOutbox(ctx context.Context, tx pgx.Tx) (string, []byte, error) {
	var taskID string
	var body []byte
	err := tx.QueryRow(ctx, `
		SELECT task_id, body FROM outbox
		ORDER BY created_at
		LIMIT 1
...
func DeleteOutbox(ctx context.Context, q Querier, taskID string) error {
	_, err := q.Exec(ctx, `DELETE FROM outbox WHERE task_id=$1`, taskID)
	return err
}
...
func MarkCompleted(ctx context.Context, q Querier, id string, r task.Result) error {
	...
	// idempotent: 既に completed なら no-op (at-least-once 配信のため)
	_, err = q.Exec(ctx, `
